...
- `in_𝘰𝘣𝘫𝘦𝘤𝘵 𝘯𝘢𝘮𝘦` this data member is a proxy object providing `begin` and `end` that allows you to write loops such as `for(auto i : container.in_𝘰𝘣𝘫𝘦𝘤𝘵 𝘯𝘢𝘮𝘦) ...`. In this loop `i` will be an appropriately typed fat handle that will iterate over each of the object instances. If the object is defined as having `erasable` storage, `i` will skip any empty positions in the underlying storage array.
