...
namespace os {
	...
	namespace internal {
		...
		class alignas(64) dummy_rel_class {
			...
			//
			// storage space for array_right of type std::vector<dummy_rel_id>
			//
			struct dtype_array_right {
				std::vector<std::vector<dummy_rel_id>> values;
				DCON_RELEASE_INLINE auto vptr() const { return values.data() + 1; }
				DCON_RELEASE_INLINE auto vptr() { return values.data() + 1; }
	...
	class alignas(64) data_container {
		...
		//
		// many key getters and setters for dummy_rel: right
		//
		...
		void dummy_rel_set_right(dummy_rel_id id, thingy2_id value) noexcept {
			if(auto old_value = dummy_rel.m_right.vptr()[id.index()]; bool(old_value)) {
				auto& vref = dummy_rel.m_array_right.vptr()[old_value.index()];
			...
			if(bool(value)) {
				dummy_rel.m_array_right.vptr()[value.index()].push_back(id);
			}
		...
		template<typename T>
		DCON_RELEASE_INLINE void thingy2_for_each_dummy_rel_as_right(thingy2_id id, T&& func) const {
			if(bool(id)) {
				auto& vref = dummy_rel.m_array_right.vptr()[id.index()];
				std::for_each(vref.begin(), vref.end(), func);
			}
		}
		DCON_RELEASE_INLINE std::pair<dummy_rel_id const*, dummy_rel_id const*> thingy2_range_of_dummy_rel_as_right(thingy2_id id) const {
			if(bool(id)) {
				auto& vref = dummy_rel.m_array_right.vptr()[id.index()];
		...
		void thingy2_remove_all_dummy_rel_as_right(thingy2_id id) noexcept {
			auto rng = thingy2_range_of_dummy_rel_as_right(id);
		...
		template<typename T>
		DCON_RELEASE_INLINE void thingy2_for_each_dummy_rel(thingy2_id id, T&& func) const {
			if(bool(id)) {
				auto& vref = dummy_rel.m_array_right.vptr()[id.index()];
				std::for_each(vref.begin(), vref.end(), func);
			}
		}
		DCON_RELEASE_INLINE std::pair<dummy_rel_id const*, dummy_rel_id const*> thingy2_range_of_dummy_rel(thingy2_id id) const {
			if(bool(id)) {
				auto& vref = dummy_rel.m_array_right.vptr()[id.index()];
				return std::pair<dummy_rel_id const*, dummy_rel_id const*>(vref.data(), vref.data() + vref.size());
			} else {
		...
		void thingy2_remove_all_dummy_rel(thingy2_id id) noexcept {
			auto rng = thingy2_range_of_dummy_rel_as_right(id);
			dcon::local_vector<dummy_rel_id> temp(rng.first, rng.second);
			std::for_each(temp.begin(), temp.end(), [t = this](dummy_rel_id i) { t->delete_dummy_rel(i); });
		}
		...
		//
		template<typename T>
		void thingy2_for_each_left_from_dummy_rel(thingy2_id id, T&& func) const {
			thingy2_for_each_dummy_rel_as_right(id, [&](dummy_rel_id i) {
				func(dummy_rel_get_left(i));
			} );
		}
		bool thingy2_has_left_from_dummy_rel(thingy2_id id, thingy_id const& target) const {
			auto& vref = dummy_rel.m_array_right.vptr()[id.index()];
		...
		void pop_back_thingy2() {
			...
			thingy2_remove_all_dummy_rel_as_right(id_removed);
			dummy_rel.m_array_right.values.pop_back();
		...
		void thingy2_resize(uint32_t new_size) {
			...
			if(new_size < old_size) {
				...
				thingy2.m_some_value.values.resize(1 + new_size);
				dummy_rel.m_array_right.values.resize(1 + new_size);
			}
		...
		thingy2_id create_thingy2() {
			...
			dummy_rel.m_array_right.values.emplace_back();
		...
		void delete_thingy2(thingy2_id id) {
			...
			thingy2_remove_all_dummy_rel_as_right(id_removed);
			thingy2_for_each_dummy_rel_as_right(last_id, [t = this, id_removed](dummy_rel_id i){ t->dummy_rel.m_right.vptr()[i.index()] = id_removed; });
			dummy_rel.m_array_right.vptr()[id_removed.index()] = std::move(dummy_rel.m_array_right.vptr()[last_id.index()]);
			dummy_rel.m_array_right.values.pop_back();
		...
		void dummy_rel_resize(uint32_t new_size) {
			...
			if(new_size < old_size) {
				...
				std::fill_n(dummy_rel.m_array_right.vptr() + 0, thingy2.size_used, std::vector<dummy_rel_id>{});
			} else if(new_size > old_size) {
			}
		...
		void delete_dummy_rel(dummy_rel_id id_removed) {
			dummy_rel.hashm_joint.erase( dummy_rel.to_joint_keydata(thingy_id(thingy_id::value_base_t(id_removed.index())), dummy_rel.m_right.vptr()[id_removed.index()]) );
		...
		void internal_move_relationship_dummy_rel(dummy_rel_id last_id, dummy_rel_id id_removed) {
			dummy_rel.hashm_joint.erase(dummy_rel.to_joint_keydata(thingy_id(thingy_id::value_base_t(last_id.index())), dummy_rel.m_right.vptr()[last_id.index()]));
			dummy_rel_set_right(id_removed, thingy2_id());
			if(auto tmp = dummy_rel.m_right.vptr()[last_id.index()]; bool(tmp)) {
				auto& vref = dummy_rel.m_array_right.vptr()[tmp.index()];
			...
			dummy_rel.hashm_joint.erase(dummy_rel.to_joint_keydata(thingy_id(thingy_id::value_base_t(last_id.index())), dummy_rel.m_right.vptr()[id_removed.index()]));
			dummy_rel.hashm_joint.insert_or_assign(dummy_rel.to_joint_keydata(thingy_id(thingy_id::value_base_t(id_removed.index())), dummy_rel.m_right.vptr()[id_removed.index()]), id_removed);
		}
						...
						if(header.is_property("$size") && header.record_size == sizeof(uint32_t)) {
							...
							for(uint32_t idx = 0; idx < dummy_rel.size_used; ++idx) {
								...
								if(dummy_rel_is_valid(dummy_rel_id(dummy_rel_id::value_base_t(idx)))) {
									auto key_dat = dummy_rel.to_joint_keydata(thingy_id(thingy_id::value_base_t(idx)), dummy_rel.m_right.vptr()[idx]);
									if(auto it = dummy_rel.hashm_joint.find(key_dat); it !=  dummy_rel.hashm_joint.end()) {
										delete_dummy_rel(it->second);
									}
	...
	class thingy2_fat_id {
		...
		template<typename T>
		DCON_RELEASE_INLINE void for_each_dummy_rel_as_right(T&& func) const;
		DCON_RELEASE_INLINE std::pair<dummy_rel_id const*, dummy_rel_id const*> range_of_dummy_rel_as_right() const;
		DCON_RELEASE_INLINE void remove_all_dummy_rel_as_right() const noexcept;
		template<typename T>
		...
		template<typename T>
		DCON_RELEASE_INLINE void for_each_left_from_dummy_rel(T&& func) const;
		DCON_RELEASE_INLINE bool has_left_from_dummy_rel(thingy_id target) const;
	};
	...
	DCON_RELEASE_INLINE std::vector<float>& thingy_fat_id::get_obj_value() const noexcept { return container.thingy_get_obj_value(id); }
	DCON_RELEASE_INLINE void thingy_fat_id::set_obj_value(std::vector<float> const& v) const noexcept { container.thingy_set_obj_value(id, v); }
	...
	DCON_RELEASE_INLINE std::vector<float> const& thingy_const_fat_id::get_obj_value() const noexcept { return container.thingy_get_obj_value(id); }
	...
	template<typename T>
	DCON_RELEASE_INLINE void thingy2_fat_id::for_each_dummy_rel_as_right(T&& func) const {
		container.thingy2_for_each_dummy_rel_as_right(id, [&, t = this](dummy_rel_id i){func(fatten(t->container, i));});
	}
	DCON_RELEASE_INLINE std::pair<dummy_rel_id const*, dummy_rel_id const*> thingy2_fat_id::range_of_dummy_rel_as_right() const {
		return container.thingy2_range_of_dummy_rel_as_right(id);
	}
	DCON_RELEASE_INLINE void thingy2_fat_id::remove_all_dummy_rel_as_right() const noexcept {
		container.thingy2_remove_all_dummy_rel_as_right(id);
	}
	template<typename T>
	DCON_RELEASE_INLINE void thingy2_fat_id::for_each_dummy_rel(T&& func) const {
		container.thingy2_for_each_dummy_rel(id, [&, t = this](dummy_rel_id i){func(fatten(t->container, i));});
	}
	DCON_RELEASE_INLINE std::pair<dummy_rel_id const*, dummy_rel_id const*> thingy2_fat_id::range_of_dummy_rel() const {
		return container.thingy2_range_of_dummy_rel(id);
	}
	DCON_RELEASE_INLINE void thingy2_fat_id::remove_all_dummy_rel() const noexcept {
		container.thingy2_remove_all_dummy_rel(id);
	}
	template<typename T>
	DCON_RELEASE_INLINE void thingy2_fat_id::for_each_left_from_dummy_rel(T&& func) const {
		container.thingy2_for_each_left_from_dummy_rel(id, [&, t = this](thingy_id i){func(fatten(t->container, i));});
	}
	DCON_RELEASE_INLINE bool thingy2_fat_id::has_left_from_dummy_rel(thingy_id target) const {
		return container.thingy2_has_left_from_dummy_rel(id, target);
	}
	...
	template<typename T>
	DCON_RELEASE_INLINE void thingy2_const_fat_id::for_each_dummy_rel_as_right(T&& func) const {
		container.thingy2_for_each_dummy_rel_as_right(id, [&, t = this](dummy_rel_id i){func(fatten(t->container, i));});
	}
	DCON_RELEASE_INLINE std::pair<dummy_rel_id const*, dummy_rel_id const*> thingy2_const_fat_id::range_of_dummy_rel_as_right() const {
		return container.thingy2_range_of_dummy_rel_as_right(id);
	}
	template<typename T>
	DCON_RELEASE_INLINE void thingy2_const_fat_id::for_each_dummy_rel(T&& func) const {
		container.thingy2_for_each_dummy_rel(id, [&, t = this](dummy_rel_id i){func(fatten(t->container, i));});
	}
	DCON_RELEASE_INLINE std::pair<dummy_rel_id const*, dummy_rel_id const*> thingy2_const_fat_id::range_of_dummy_rel() const {
		return container.thingy2_range_of_dummy_rel(id);
	}
	template<typename T>
	DCON_RELEASE_INLINE void thingy2_const_fat_id::for_each_left_from_dummy_rel(T&& func) const {
		container.thingy2_for_each_left_from_dummy_rel(id, [&, t = this](thingy_id i){func(fatten(t->container, i));});
	}
	DCON_RELEASE_INLINE bool thingy2_const_fat_id::has_left_from_dummy_rel(thingy_id target) const {
		return container.thingy2_has_left_from_dummy_rel(id, target);
	}
